---
description: 跟踪 Claude Code subagents 报告变更并查找需要更新的内容
argument-hint: [要检查的版本数，默认 10]
---

# Workflow Changelog — Subagents Report

你是 claude-code-best-practice 项目的协调员。你的任务是启动一个研究 agent，等待其结果，并呈报 **Subagents Reference** 报告（`best-practice/claude-subagents.md`）的漂移情况。

此 workflow 检查的正好是**两种漂移类型**：
1. **Frontmatter 字段** — 官方文档中新增或移除的任何字段
2. **Official sub-agents** — 新增或移除的任何 built-in agent
...
使用以下 prompt 启动 `workflow-claude-subagents-agent`：

> 研究 claude-code-best-practice 项目中 subagents 报告的漂移情况。检查最近 $ARGUMENTS 个版本（默认 10）。
>
> 获取以下 2 个外部来源：
...
>
> 检查两件事：
> 1. **Frontmatter 字段**：将官方文档支持的 frontmatter 字段表与报告的 Frontmatter Fields 表进行比较。标记任何新增或移除的字段。
> 2. **Official sub-agents**：将官方文档的 built-in subagents 列表与报告的 official agents 表进行比较。标记任何新增或移除的 agents。

---

## Phase 2: 读取之前的 Changelog 条目

**在 agent 运行期间**，读取 `changelog/best-practice/claude-subagents/changelog.md` 获取最近 25 条条目。解析 priority actions 以识别：
...
2. **Official Sub-agent Changes** — 与表格相比新增或移除的 built-in agents
...
读取现有的 `changelog/best-practice/claude-subagents/changelog.md` 文件，然后**追加**（不要覆盖）一个新条目在末尾。条目格式必须严格为：
...
- 如果 `changelog/best-practice/claude-subagents/changelog.md` 不存在或为空，先创建带有 Status Legend 表（参见文件顶部），然后是第一个条目
- 每个条目之间用 `---` 分隔
...
更新 `best-practice/claude-subagents.md` 顶部的 "Last Updated" badge。运行 `TZ=Asia/Karachi date "+%b %d, %Y %-I:%M %p PKT"` 获取时间，对其进行 URL 编码（空格转 `%20`，逗号转 `%2C`），然后替换 badge 中的日期部分。如果版本号有变化，同时更新 badge 中的 Claude Code 版本。
...
- **New agents**：添加到 official agents 表，包含正确的 #、name、model、tools 和 description
- **Removed agents**：移除前与用户确认
...
3. **交叉验证 agent 计数** — 报告的 agent 数必须与官方文档一致
4. **不要自动执行** — 始终先呈报报告
